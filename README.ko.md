...
한 번 등록하면 됩니다. 모든 세션이 자동으로 요약, 인덱싱, 검색 가능해집니다. 기억할 명령어도, 들일 습관도 없습니다.
...
문제가 발생하면 `register`를 다시 실행하세요 — 환경을 확인하고 일반적인 문제를 자동 수정합니다:
...
로그는 기본적으로 개인정보(경로, 사용자 이름 등)가 자동 마스킹됩니다. GitHub issue에 바로 붙여넣을 수 있습니다. 원본 로그가 필요하면 `--no-mask`를 추가하세요.
